...
import { AttribList, UniformList } from "../materials/GLShader";
...
/**
 * Implementation of a really cool futuristic looking shader post-processing effect found online. 
 * TODO: Add the link where I found the effect 
 */
export class Effect2DGlitch extends Effect2D {

    constructor(name:string, uuid?:string) {
        super(name, DrawStaticPrimitiveType.TRIANGLES, uuid); 

        this.material = new Effect2DSurface("Effect2DGlitch");
        this.material.useInputTexture = true; 
        this._setShaderData(); 
        this._genBuffers(); 
    }

    private _setShaderData():void {
        ...
        void main() { 
           gl_Position = vec4(position * vec2(1, flipY), 0, 1);
           vUv = texCoord;
        }
        `;
    ...
    const fShader = `precision highp float;
        ...
        /* This has to be accessible by .x and .y in the glsl shader. vec2 from glmatrix don't do this */
        customUniformList.resolution = {
            type: "vec2",
            uniformLocation: -1,
            update: true,
            value: null
        ...
        this.material.setCustomShader("Effect2DGlitchShader", vShader, fShader, customUniformList, customAttribList); 
    }
    ...
    private _genBuffers():void {
        ...
        glPositionBuffer.setAttribPtrData(2, false, 0, 0);
